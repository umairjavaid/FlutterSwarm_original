    ...
    def _write_flutter_project(self, project_name: str) -> str:
        ...
        try:
            ...
            if not flutter_tool:
                ...
                return ""
            # Run the async execute method in a synchronous context
            loop = asyncio.get_event_loop()
            if loop.is_running():
                coro = flutter_tool.execute("create_project", params)
                result = asyncio.ensure_future(coro)
                # In a running event loop, we can't block, so just log and return stub for now
                logger.error("Cannot synchronously wait for Flutter project creation in running event loop.")
                return ""
            else:
                result = loop.run_until_complete(flutter_tool.execute("create_project", params))
            if result.status.name == "SUCCESS":
